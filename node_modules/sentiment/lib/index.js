...
module.exports = function (phrase, inject, callback) {
    ...
    */
    ...
    var relevant_words_weight = [3,3,3,3,3,3,3,3,3,3,3];
    var score=0;
    console.log("VSM:");
    ...
    */
    ...
    console.log("Cosine similarity:");
    var cosineAngle_matrix = [];
    var totalTweets=vsm.length;
    console.log("totalTweets="+totalTweets);
    for(var i=0; i<totalTweets-1; ++i){
      var v1 = vsm[i];
      for(var j=i+1; j<totalTweets ; ++j){
        var v2 = vsm[j];
        cosineAngle_matrix.push([i,j,cosineSimilarity(v1,v2)]);
      }
    }
    ...
    //LSA using SVD
    var svd = require('node-svd').svd;
    var res = svd(vsm, 0, { U: true, V: true, debug: 1});
    console.log('---');
    var U = res.U;
    var S = res.S;
    var V = res.V;
    ...
    var feature2Words = [];
    for (i=0; i<rows; ++i){
      if(VS[i][0] > 0){ //cloumn 0 for feature1
        feature1Words.push(relevant_words[i]);
      }
      if(VS[i][1] > 0){ //cloumn 1 for feature2
        feature2Words.push(relevant_words[i]);
      }
    }
    console.log("Feature1 words = "+feature1Words);
